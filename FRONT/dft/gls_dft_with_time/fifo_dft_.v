...
module testbench;
  ...
  //-----------------------------------------------------------
  // DFT/EDT pins on the gate-level top, tied to their inactive
  // ("functional mode") state -- this run only checks functional
  // correctness of the post-DFT/EDT netlist, it does not exercise
  // scan shifting or EDT compression/decompression.
  //-----------------------------------------------------------
  wire scan_en            = 1'b0;  // 0 = functional data path (not scan chain)
  wire edt_clock           = 1'b0;  // EDT controller/decompressor idle
  wire edt_update          = 1'b0;
  wire edt_bypass          = 1'b1;  // bypass EDT compression logic entirely
  wire edt_channels_in1    = 1'b0;
  wire edt_channels_in2    = 1'b0;
  wire edt_channels_in3    = 1'b0;
  wire edt_channels_out1, edt_channels_out2, edt_channels_out3; // unused outputs

  // DUT - gate-level netlist, post-synthesis + DFT/EDT insertion
  //-----------------------------------------------------------
  sync_fifo_edt_top dut (
    .clk_i             (clk_i),
    .rst_i             (rst_i),
    .wr_en_i           (wr_en_i),
    .rd_en_i           (rd_en_i),
